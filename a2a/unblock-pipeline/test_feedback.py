#!/usr/bin/env python3
"""
배정 → 리포트 → 가이드 → 초안 → 피드백 5-step 테스트.

데미안이 기자+팀장을 배정하고, 기자가 시장 조사 리포트를 작성,
팀장이 가이드를 작성, 기자가 초안을 작성한 뒤,
팀장이 초안에 대해 피드백을 제공한다.

사용:
  python3 test_feedback.py                      # 랜덤 샘플
  python3 test_feedback.py --topic regulation   # 특정 주제
  python3 test_feedback.py --source article.txt # 직접 원문
  python3 test_feedback.py --base-url http://localhost:3000
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="배정 → 리포트 → 가이드 → 초안 → 피드백 5-step 테스트"
    )
    ...
    print("=" * 60)
    print(f" 5-Step 테스트  |  주제: {topic}  |  날짜: {today}")
    print("=" * 60)
    print(f"\n[원문]\n{source}\n")
    ...
    return 0 if report.strip() and guide.strip() and draft.strip() and feedback.strip() else 1
