#!/usr/bin/env python3
"""
배정 → 리포트 → 가이드 → 초안 → 피드백 → 수정 → 승인 7-step 테스트.

전체 편집 파이프라인을 실행한다. 마지막에 데미안(편집국장)이
수정된 기사를 승인하거나 반려한다.

사용:
  python3 test_confirm.py                      # 랜덤 샘플
  python3 test_confirm.py --topic regulation   # 특정 주제
  python3 test_confirm.py --source article.txt # 직접 원문
  python3 test_confirm.py --base-url http://localhost:3000
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="배정 → 리포트 → 가이드 → 초안 → 피드백 → 수정 → 승인 7-step 테스트"
    )
    ...
    print("=" * 60)
    print(f" 7-Step 테스트  |  주제: {topic}  |  날짜: {today}")
    print("=" * 60)
    print(f"\n[원문]\n{source}\n")
    ...
    # ── Step 7: 편집국장 최종 승인/반려 ──────────────────────
    print("\n" + "─" * 60)
    print(f"STEP 7 — 데미안 최종 승인/반려 (skill=confirm)")
    print("─" * 60)
    ...
    # 승인/반려 판정
    verdict = "REJECTED" if "반려" in confirm else "APPROVED" if any(k in confirm for k in ("승인", "발행")) else "UNKNOWN"
