#!/usr/bin/env python3
"""
배정 → 리포트 → 가이드 → 초안 → 피드백 → 수정 6-step 테스트.

데미안이 기자+팀장을 배정하고, 기자가 시장 조사 리포트를 작성,
팀장이 가이드를 작성, 기자가 초안을 작성, 팀장이 피드백을 준 뒤,
기자가 피드백을 반영하여 기사를 수정한다.

사용:
  python3 test_revision.py                      # 랜덤 샘플
  python3 test_revision.py --topic regulation   # 특정 주제
  python3 test_revision.py --source article.txt # 직접 원문
  python3 test_revision.py --base-url http://localhost:3000
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="배정 → 리포트 → 가이드 → 초안 → 피드백 → 수정 6-step 테스트"
    )
    ...
    print("=" * 60)
    print(f" 6-Step 테스트  |  주제: {topic}  |  날짜: {today}")
    print("=" * 60)
    print(f"\n[원문]\n{source}\n")
    ...
    # ── Step 4: 기자 초안 작성 ───────────────────────────────
    print("\n" + "─" * 60)
    print(f"STEP 4 — {rep['kor']} 초안 작성 (skill=writing)")
    print("─" * 60)
    ...
    # ── Step 5: 팀장 피드백 ──────────────────────────────────
    print("\n" + "─" * 60)
    print(f"STEP 5 — {mgr['kor']} 피드백 (skill=feedback)")
    print("─" * 60)

    feedback = call_agent(
        args.base_url, manager_id,
        user_text=f"{rep['kor']} 기자가 작성한 기사에 대해 피드백을 줘.",
        skill_id="feedback",
        variables={
            "REPORTER": rep["kor"],
            "TODAY_DATE": today,
    ...
    # ── Step 6: 기자 수정 ────────────────────────────────────
    print("\n" + "─" * 60)
    print(f"STEP 6 — {rep['kor']} 수정 (skill=revision)")
    print("─" * 60)
    ...
    print(f"  초안 길이:   {len(draft.strip())}자  {'✓' if draft.strip() else '✗'}")
    print(f"  피드백 길이:  {len(feedback.strip())}자  {'✓' if feedback.strip() else '✗'}")
    print(f"  수정본 길이:  {len(revision.strip())}자  {'✓' if revision.strip() else '✗'}")
    print("=" * 60)

    return 0 if report.strip() and guide.strip() and draft.strip() and feedback.strip() and revision.strip() else 1
