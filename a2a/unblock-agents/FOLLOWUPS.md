...
## Slice B — web search integration (✅ IMPLEMENTED)
...
- Provider: Tavily, same as unblockmedia-backend. Parameters match the
  parent: `topic: 'news'`, `max_results: 10`.
- Query generation: reuses `callLLM` (no separate model). Single Korean
  query prompt (English variant in the parent is dropped because our
  pipeline is Korean-only).
- Applied to the `report` skill **only**. Other stages operate on
  already-provided inputs (market research, draft, feedback) and don't
  need fresh external context — confirmed by Notion prompt analysis.
- Silent degradation: missing `TAVILY_API_KEY` or a failed Tavily call
  returns empty search context; the executor composes the system
  prompt without a `<Web Search Results>` section and proceeds.
- Delivered in `src/lib/search.ts` (Tavily wrapper + `searchForReport`)
  and one hook in `src/lib/executor.ts::execute()`.
...
| Slice | Fixes                          | Code   | External cost     | Risk    | Status |
| ----- | ------------------------------ | ------ | ----------------- | ------- | ------ |
| **A** | `^VAR^` literal leaks          | ~30 LoC | none              | low     | ✅ done |
| **B** | number/fact hallucination      | ~90 LoC | Tavily key        | medium  | ✅ done |
| **C** | tone bleed into skill outputs  | ~15 LoC | none              | medium  | pending |

Recommended order: **A → B → C**. A and B are shipped; C is the
remaining quality lever — with real numbers in and placeholders
out, persona bleed is now the most visible issue.
