...
export default function TopBar() {
  ...
  return (
    <div
      className="flex items-center h-10 px-2 shrink-0 border-b border-black/30"
    ...
    >
      ...
      <div className="flex items-center gap-1 w-1/4 min-w-0">
        <button
          ...
          disabled={!canBack}
          className={cn(
            'flex items-center justify-center w-7 h-7 rounded text-white/80 transition-colors',
        ...
        <button
          ...
          className="flex items-center justify-center w-7 h-7 rounded text-white/80 hover:bg-white/10 hover:text-white transition-colors"
          title="Recent"
          aria-label="Recent"
        >
      ...
      <div className="flex-1 flex items-center justify-center">
        <button
          onClick={() => setSearchOpen(true)}
          className="w-full max-w-[720px] flex items-center gap-2 h-7 px-3 rounded-md bg-black/15 hover:bg-black/25 border border-white/10 hover:border-white/20 text-left transition-colors group"
          aria-label="Search"
        >
          ...
          <kbd className="hidden md:inline text-[10px] text-white/50 border border-white/15 rounded px-1 py-0.5 font-mono group-hover:text-white/70 transition-colors">
            /
          </kbd>
        </button>
      ...
      <div className="flex items-center gap-1 w-1/4 justify-end min-w-0">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="relative flex items-center gap-1 h-7 px-1 rounded hover:bg-white/10 transition-colors focus:outline-none">
            <Avatar className="w-6 h-6">
              {user?.avatarUrl && <AvatarImage src={user.avatarUrl} alt={user.displayName} />}
              <AvatarFallback className="bg-[#611f6a] text-white text-[10px] font-semibold">
                {initials}
              </AvatarFallback>
            </Avatar>
            <span
