...
import { TemplateGallery } from '@/components/notion/template-gallery';
            ...
            <SidebarSection label="Recent" icon={<Clock size={12} aria-hidden="true" />}>
              {recentPages.slice(0, 5).map((r) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => router.push(`/notion/workspace/${workspaceId}/${r.pageId}`)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      router.push(`/notion/workspace/${workspaceId}/${r.pageId}`);
                    }
                  }}
