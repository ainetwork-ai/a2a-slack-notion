...
export function TemplateGallery({ workspaceId, parentId, onClose }: TemplateGalleryProps) {
  ...
  useEffect(() => {
    apiFetch<{ templates: PageTemplate[] }>(`/api/page-templates?workspaceId=${workspaceId}`)
  ...
  async function handleBlankPage() {
    try {
      ...
      const newPage = await apiFetch<{ id: string }>(
        `/api/notion/pages?${params.toString()}`,
        { method: 'POST', body: JSON.stringify({ title: 'Untitled' }) },
      );
      onClose();
      router.push(`/notion/${workspaceId}/${newPage.id}`);
    } catch (err) {
  ...
  async function handleApplyTemplate(template: PageTemplate) {
    try {
      ...
      if (parentId) params.set('parentId', parentId);
      const newPage = await apiFetch<{ id: string }>(
        `/api/page-templates/${template.id}/apply?${params.toString()}`,
        { method: 'POST' },
      );
      onClose();
      router.push(`/notion/${workspaceId}/${newPage.id}`);
