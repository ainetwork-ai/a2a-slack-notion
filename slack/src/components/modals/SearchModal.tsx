...
import { Hash, Lock, MessageSquare, User, Loader2, Search, X, ChevronDown, Clock, Trash2 } from 'lucide-react';
import { useAppStore } from '@/lib/stores/app-store';
import { useSearch } from '@/lib/hooks/use-search';
import { useRecentVisits, clearRecentVisits } from '@/lib/hooks/use-recent-visits';
          ...
          <div className="max-h-80 overflow-y-auto scrollbar-slack">
            ...
            {!query && !isSearching && (
              <div>
                  ...
                  <div>
                    ...
                    {recentVisits.map((v) => (
                      <button
                        ...
                        onClick={() => {
                          if (v.type === 'channel') {
                            router.push(`/workspace/channel/${encodeURIComponent(v.id)}`);
                          } else {
                            router.push(`/workspace/dm/${encodeURIComponent(v.id)}`);
                          }
                          handleClose();
                        }}
                        className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-slate-200 hover:bg-white/10 transition-colors text-left"
                      >
                        {v.type === 'channel' ? (
                          v.isPrivate ? (
                ...
                {recentVisits.length === 0 && history.length === 0 && (
