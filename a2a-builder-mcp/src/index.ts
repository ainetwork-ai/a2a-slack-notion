...
server.tool(
  "create_agent",
  "Scaffold a new A2A (Agent-to-Agent) protocol agent with Hono server, agent card, and local vLLM (Gemma 4) task handler",
  {
...
server.tool(
  "test_agent",
  "Start an A2A agent locally and run protocol compliance tests (agent card, tasks/send, tasks/get)",
  {
...
server.tool(
  "deploy_agent",
  "Deploy an A2A agent to Vercel and return the agent card JSON URL",
  {
    name: z.string().describe("Agent slug name"),
...
server.tool(
  "list_agents",
  "List all created A2A agents with their status (installed, built, deployed)",
  {},
  async () => {
    try {
