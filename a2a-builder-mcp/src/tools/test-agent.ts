...
async function waitForServer(url: string, timeoutMs = 10000): Promise<boolean> {
...
export async function testAgent(input: TestAgentInput): Promise<string> {
  ...
  try {
    ...
    if (!cardRes.ok) {
      ...
      results.push(`  Skills: ${card.skills?.map((s: any) => s.name).join(", ") || "none"}`);
    }
    ...
    if (!sendRes.ok) {
      ...
      if (sendBody.error) {
        ...
        const text =
          sendBody.result?.status?.message?.parts?.[0]?.text || "(no text)";
