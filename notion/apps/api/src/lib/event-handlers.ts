 ...
 *
 * Previous design used BullMQ + Redis to queue notification inserts and
 * webhook deliveries. Redis was removed from the Notion stack — the app is
 * single-instance and the work is light, so we just run the handlers in-process.
 *
 * For mentions/comments: insert a `notionNotifications` row directly and push
 * an SSE event to any subscribed client.
 * For webhooks: fan out an HTTP POST to every registered webhook that listens
 * for the event, signed with HMAC-SHA256. Fire-and-forget (no retry today).
 */
